...
// Builder provides a fluent interface for creating plugins
type Builder struct {
	info     Info
	features []string
}
...
func (b *Builder) WithManual(manual string) *Builder {
	b.info.Manual = manual
	return b
}

// WithSupport sets the support URL
func (b *Builder) WithSupport(support string) *Builder {
	b.info.Support = support
	return b
}

// AddFeature adds a plugin feature
func (b *Builder) AddFeature(feature string) *Builder {
	b.features = append(b.features, feature)
	return b
}

// Build returns the completed plugin info
func (b *Builder) Build() Info {
	b.info.Features = b.features
	return b.info
}
